...
def PlayWrapper(command):
    async def wrapper(client, message):
        ...
        # Normalize Turkish commands to English equivalents for parsing
        raw_cmd = message.command[0].lower()
        # Map Turkish oynat variants to play variants
        normalized_cmd = raw_cmd
        if "oynat" in raw_cmd:
            normalized_cmd = raw_cmd.replace("oynat", "play")
        
        if normalized_cmd[0] == "c":
        ...
        if normalized_cmd[0] == "v":
            video = True
        else:
            if "-v" in message.text:
                video = True
            elif len(normalized_cmd) > 1 and normalized_cmd[1] == "v":
                video = True
            else:
                video = None
        if normalized_cmd[-1] == "e":
